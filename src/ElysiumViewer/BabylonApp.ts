...
import { Tools, RenderTargetTexture, RenderTargetWrapper} from "@babylonjs/core/";
...
import {GetWebOptions} from "./ViewerUtility";
import EventSystem from "../Utility/EventSystem";
import {EventTag, TexturePath, MaterialParameters, WebsiteOption} from "./GeneralStaticFlag";
...
export default class BabylonApp {
    ...
    private m_webOptions : WebsiteOption;
    ...
    constructor(canvasDOM: HTMLCanvasElement, eventSystem: EventSystem) {
        let self = this;
        this.m_webOptions = GetWebOptions();

        this.m_loadingScreen = new LoadingScreenView("", this.m_webOptions);
        ...
        this.m_mainScene = new MainSceneHandler(this.m_engine, this.m_canvasDOM, this.m_webOptions, this.m_loadingScreen, this.m_eventSystem);
    ...
    public async TakeScreenshot(screenImgDom: HTMLImageElement) {
        let width = 2048;
        let height = width * (this.m_canvasDOM.height / this.m_canvasDOM.width);

        if (this.m_canvasDOM.height >  this.m_canvasDOM.width) {
            height = 2048;
            width = height * (this.m_canvasDOM.width / this.m_canvasDOM.height);    
        }

        let origin_height = this.m_canvasDOM.height;
        let origin_width = this.m_canvasDOM.width;
        ...
        this.m_engine.setSize(width, height, true);

        let data_url = await Tools.CreateScreenshotAsync(this.m_engine, this.MainScene.Camera, 
                                                {width:this.m_canvasDOM.width, height: this.m_canvasDOM.height}, "image/jpg");      
                                                
        this.m_engine.setSize(origin_width, origin_height, true);

        screenImgDom.src = data_url;
        screenImgDom.style.height = "110px";
        screenImgDom.style.width =  (this.MainScene.AspectRatio * 110)+ "px";
    ...
    private OnSceneReady() {
        ...
        this.m_free_style_mode = new FreeStyleMode(ModeEnum.FreeStyle, this.m_mainScene.Camera, this.m_mainScene.CharacterMesh.GetMainMesh, FreeStyleLerpStruct(this.m_webOptions));
    ...
    private ProcessMousePicking() {
        if (this.m_mainScene == null || this.m_mainScene.Scene == undefined || this.m_mainScene.CharacterMesh == undefined || this.m_webOptions.is_website) return;
        ...
        if (pickResult.hit) {
            ...
            this.m_emojiSystem.ShowRandomEmoji(new Vector3(0, bounding.maximum.y - 0.15,0 ), this.m_mainScene.Camera.absoluteRotation);
